...
from kinesinlms.course.custom_views.views import (
...
from kinesinlms.custom_app.views import (
    course_speakers,
    peer_review_journal,
    simple_html_content,
)
...
class CourseViewSet(viewsets.ViewSet):
    ...
    @action(detail=True, methods=["get"], name="Course Assessments and Activities")
            ...
            for section_node in module_node.children.all():
                for unit_node in section_node.children.all():
                    ...
                    for unit_block in course_unit.unit_blocks.all():
                        if unit_block.block.type not in [
                            BlockType.ASSESSMENT.name,
...
def course_home_page(request, course_slug=None, course_run=None):
    ...
    Tracker.track(
        event_type=TrackingEventType.COURSE_HOME_VIEW.value,
        user=request.user,
...
def redirect_to_unit_page(request, course_slug, course_run, module_slug=None, section_slug=None):
    ...
    if not module_slug:
        ...
        if last_viewed_unit_id:
            try:
                ...
                logger.exception(
                    f"Tried to use last_viewed_unit_id {last_viewed_unit_id} in session "
    ...
    if not redirect_url:
        ...
        try:
            ...
            logger.exception(
                f"Could not generate redirect_url for {course.token} from "
...
def unit_page(
    ...
    *not* the Block object it refers to.)
    ...
    # Collect any extra context from relevant blocks in the unit
    extra_context = {}
    ...
    if unit_is_released:
        ...
        if course_unit.type in [
            ...
            CourseUnitType.SECTION_LEARNING_OBJECTIVES.name,
            ...
            if sit_blocks.count() > 0:
                for sit_block in sit_blocks:
                    ...
                    for js_library in js_libraries:
                        if js_library not in extra_course_unit_js_libraries:
                            extra_course_unit_js_libraries.append(js_library)
            # If any LTI blocks use an iframe, we have to add the appropriate CSP to the header.
            # We'll do this by adding the CSPs to the context and the base.html template will
            # write them out if they're defined in the 'content_security_policies' variable.
            lti_iframe_blocks = blocks.filter(
    ...
    if unit_is_released:
        # Track activity
        Tracker.track(
            event_type=TrackingEventType.COURSE_PAGE_VIEW.value,
            user=request.user,
    ...
    try:
        cookie_name = f"{course.token}_last_viewed_unit_id"
        response.set_cookie(cookie_name, str(current_unit_node.id), samesite="Lax")
    except Exception:
...
def course_search_page(request, course_slug, course_run):
    ...
    Tracker.track(
        event_type=TrackingEventType.COURSE_SEARCH_REQUEST.value,
        user=request.user,
        event_data={"search_text": search_text},
        course=course,
    )
...
def certificate_page(request, course_slug, course_run):
    ...
    try:
        ...
        return render(
            request,
            "course/certificate/no_certificate_template_defined_for_course.html",
            context,
        )
    ...
    Tracker.track(
        event_type=TrackingEventType.COURSE_CERTIFICATE_VIEW.value,
        user=request.user,
...
def forum_topics_page(request, course_slug=None, course_run=None):
    ...
    Tracker.track(
        event_type=TrackingEventType.COURSE_FORUM_TOPICS_VIEW.value,
        user=request.user,
...
def bookmarks_page(request, course_slug=None, course_run=None):
    ...
    Tracker.track(
        event_type=TrackingEventType.COURSE_BOOKMARKS_VIEW.value,
        user=request.user,
...
def progress_overview_page(request, course_slug=None, course_run=None):
    ...
    for milestone in milestones:
        progress: MilestoneProgress = MilestoneProgress.objects.filter(
            course=course, milestone=milestone, student=request.user
        ).first()
    ...
    Tracker.track(
        event_type=TrackingEventType.COURSE_PROGRESS_VIEW.value,
        user=request.user,
...
def progress_detail_page(request, course_slug=None, course_run=None, module_id: int = None):
    ...
    if module_id:
        ...
        progress_status = get_progress_status(
            course=course, student=request.user, target_module_node=current_module_node
        )
    else:
...
def custom_app_page(request, course_slug=None, course_run=None, custom_app_slug=None):
    ...
    Tracker.track(
        event_type=TrackingEventType.COURSE_CUSTOM_APP_PAGE_VIEW.value,
        user=request.user,
        event_data={"custom_app": custom_app.display_name},
        course=course,
        unit_node_slug=None,
        course_unit_id=None,
        course_unit_slug=None,
...
def shortcut_to_assessment(request, course_slug: str, course_run: str, unit_block_slug: str):
    ...
    if not unit_url:
        logger.exception(
            f"Could not find short for course_slug:{course_slug} "
...
def download_course_resource(
    ...
    try:
        ...
        Tracker.track(
            event_type=TrackingEventType.COURSE_RESOURCE_DOWNLOAD.value,
            user=request.user,
...
def download_resource(request, course_slug: str, course_run: str, block_resource_id: int):
    ...
    try:
        ...
        Tracker.track(
            event_type=TrackingEventType.COURSE_BLOCK_RESOURCE_DOWNLOAD.value,
            user=request.user,
            event_data=event_data,
            course=course,
...
def toggle_bookmark_hx(
    ...
    try:
        bookmark, created = Bookmark.objects.get_or_create(
            student=request.user, course=course, unit_node_id=unit_nav_info.unit_node_id
        )
    except Exception:
...
def forum_topic_posts_hx(request, course_slug=None, course_run=None, forum_topic_id: int = None):
    ...
    try:
        course, enrollment, course_nav, unit_nav_info = process_course_hx_request(
            request, course_slug=course_slug, course_run=course_run
        )
        topic = get_object_or_404(ForumTopic, topic_id=forum_topic_id)
        service: BaseForumService = get_forum_service()
        if not service.user_can_view_topic(forum_topic=topic, user=request.user):
