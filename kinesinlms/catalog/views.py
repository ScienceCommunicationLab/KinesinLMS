...
from kinesinlms.catalog.service import do_enrollment, StudentAlreadyEnrolled, EnrollmentPeriodHasNotStarted, \
    do_unenrollment
from kinesinlms.course.models import Enrollment, Course, Milestone, EnrollmentSurveyCompletion
...
from kinesinlms.course.views import get_course_nav, CourseNavException
    ...
    if request.user.is_staff or request.user.is_superuser:
        ...
        try:
            ...
            if request.user.is_authenticated and hasattr(request.user, 'enrollments'):
                try:
                    show_course_description_ids = [enrollment.course.catalog_description.id for enrollment in
                                                   request.user.enrollments.all()]
                    extra_course_descriptions = CourseCatalogDescription.objects.filter(visible=False,
                                                                                        id__in=show_course_description_ids)
...
def criteria_page(request, course_slug: str = None, course_run: str = None):
    ...
    breadcrumbs = [
        ...
        {
            "url": f"/catalog/{course.slug}/{course.run}",
            "label": f"About {course.token}"
        },
    ]
    ...
    try:
        ...
        for module_node in module_nodes:
            num_assessments = get_num_block_type_for_node(module_node,
                                                          BlockType.ASSESSMENT,
                                                          graded_only=False)
            num_graded_assessments = get_num_block_type_for_node(module_node,
                                                                 BlockType.ASSESSMENT,
                                                                 graded_only=True)

            num_sits = get_num_block_type_for_node(module_node,
                                                   BlockType.SIMPLE_INTERACTIVE_TOOL,
                                                   graded_only=False)
            num_graded_sits = get_num_block_type_for_node(module_node,
                                                          BlockType.SIMPLE_INTERACTIVE_TOOL,
                                                          graded_only=True)
    ...
    return render(request,
                  'catalog/about_course_criteria.html',
                  context)
...
def enroll(request, course_slug=None, course_run=None):
    ...
    if not request.user.is_superuser and not request.user.is_staff:
        if target_course.admin_only_enrollment:
            logger.error(f"Student {request.user} tried to enroll "
                         f"in admin only enroll course: {target_course}")
            msg = "This course is not open for enrollment. You " \
                  "must be manually enrolled by the KinesinLMS staff."
    ...
    try:
        ...
        logger.error(f"enroll(): EnrollmentPeriodHasNotStarted: "
                     f"Could not enroll {request.user} in course {target_course}.")
        msg = "The enrollment period for this course has not yet started."
        messages.add_message(request, messages.ERROR, msg)
    except StudentAlreadyEnrolled:
        logger.info(f"enroll(): StudentAlreadyEnrolled: Could not enroll "
                    f"{request.user} in course {target_course}.")
    ...
    if enrollment and enrollment.enrollment_survey_required:
        return redirect("catalog:enrollment_survey",
                        course_slug=target_course.slug,
                        course_run=target_course.run)
    else:
        return redirect("catalog:about_page",
                        course_slug=target_course.slug,
                        course_run=target_course.run)
...
def enrollment_survey(request, course_slug=None, course_run=None):
    ...
    target_course = get_object_or_404(Course,
                                      slug=course_slug,
                                      run=course_run)
    e_survey = getattr(target_course, 'enrollment_survey', None)
    if not user_is_enrolled(user=request.user, course=target_course) \
            or e_survey is None:
        return redirect("catalog:about_page",
                        course_slug=target_course.slug,
                        course_run=target_course.run)
    ...
    try:
        ...
        enrollment = None
    if enrollment and not enrollment.enrollment_survey_required:
        return redirect("course:course_home_page",
                        course_slug=target_course.slug,
                        course_run=target_course.run)
    ...
    if request.method == "GET":
        ...
        return render(request,
                      'catalog/enrollment_survey/enrollment_survey.html',
                      context)
        ...
        if enrollment_answer_form.is_valid():
            ...
            EnrollmentSurveyCompletion.objects.get_or_create(
                enrollment_survey=target_course.enrollment_survey,
...
def unenroll(request, course_slug=None, course_run=None):
    ...
    try:
        ...
        logger.exception(f"unenroll(): Error trying to unenroll student : {student} from"
                         f"course {course} ")
        msg = "There was an error when trying to unenroll you from this course. " \
              "Please try again or support for help."
        messages.add_message(request, messages.ERROR, msg)
        return redirect("catalog:about_page", course_slug=course.slug, course_run=course.run)
