...
from kinesinlms.marketing.forms import TrainerInfoForm, ContactForm
...
def index(request):
    ...
    if request.user.is_authenticated and hasattr(request.user, 'enrollments'):
        try:
            ...
            if enrollments:
                show_course_description_ids = [enrollment.course.catalog_description.id
                                               for enrollment
                                               in enrollments
...
def honor_code(request):
    context = {
        "title": "Honor Code",
        "description": "The honor code that guides your work on KinesinLMS."
    }
...
def privacy_policy(request):
    context = {
        ...
        "description": "The privacy policy for the KinesinLMS website."
    }
...
def get_started(request):
    ...
    context = {
        "title": "Get Started",
        "description": f"How to get started using {site_name}."
    }
...
def about(request):
    ...
    context = {
        "title": "About",
        "description": f"A bit about {site_name}."
    }
...
def contact(request):
        ...
        if form.is_valid():
            ...
            try:
                ...
                email_body = f"Username : {username} \n" \
                             ...
                             f"Message:\n---------------------------------\n" \
                             f"{message}\n\n\n\n---------------------------------\n\n"
                send_mail("Contact form submission from kinesinlms.org",
                          email_body,
                          "no-response@kinesinlms.org",
                          ["courses@kinesinlms.org", "admin@kinesinlms.org"])
            except Exception:
                logger.exception(f"Could not email contact form submission: "
    ...
    context = {
        'form': form,
        "title": "Contact Us",
        "description": "Contact the team at KinesinLMS."
    }
...
def trainer_info(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                message = "Someone submitted the form indicating they're interested " \
                ...
                send_mail("Inquiry: requiring courses for trainees",
                          message,
                          "courses@kinesinlms.org",
                          ['admin@kinesinlms.org'])
                try:
                    Tracker.track(event_type=TrackingEventType.ADMIN_KINESINLMS_TRAINER_FORM_SUBMITTED.value,
                                  user=request.user,
                ...
                messages.error(request,
                               message="Couldn't process form. Please contact courses@kinesinlms.org directly.")
    ...
    for cookie_name in request.COOKIES.keys():
        ...
        if cookie_name.startswith('_ga') or cookie_name.startswith('_gid') or cookie_name.startswith('_gat'):
            ...
            response.set_cookie(cookie_name,
                                domain=domain_with_starting_period,
                                value='',
                                max_age=0)
