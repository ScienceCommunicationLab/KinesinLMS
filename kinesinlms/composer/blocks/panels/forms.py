        ...
        self.helper.layout = Layout(
            ...
            HTML(
                f"""
                    ...
                    <label for="id_target_link_uri">Default Target Link URI</label>
                    <div class="alert alert-secondary mt-2">{self.instance.default_target_link_uri or '(none defined)'}</div>
                    <small class="form-text text-muted">This is the default target link (if any) defined in the ExternalTool Provider. 
                    (Defining a custom target_link_uri above will override this value).</small>
                    </div>
                """
            ),
        )
