...
def start_student_progress_report_task(student_progress_report: StudentProgressReport) -> int:
    ...
    logger.info(f"start_building_analytics_task() : starting new async task "
                f"for StudentProgressReport {student_progress_report.id}")
    async_result: AsyncResult = generate_student_progress_report.apply_async(args=[],
                                                                             kwargs={
                                                                                 'student_progress_report_id': student_progress_report.id
                                                                             },
                                                                             countdown=5)  # Give DB a chance to complete this transaction
    ...
    logger.info(f"start_building_analytics_task() : async task for StudentProgressReport "
...
def generate_student_progress_report_error_handler(self, exc, task_id, args, kwargs, einfo):  # noqa: F841
    ...
    logger.error(f"generate_student_progress_report_error_handler() "
...
def generate_student_progress_report(self, student_progress_report_id):
    ...
    """
    logger.debug(f"TASK generate_student_progress_report(): Task id: {self.request.id} : Running analytics task "
    ...
    try:
        ...
        save_result(student_progress_report=student_progress_report,
    ...
    try:
        modules_info, students_modules_progresses = get_student_module_progress(student_progress_report.course,
                                                                                students,
    ...
    try:
        ...
        student_progress_report.students_modules_progresses_json = [smp.to_dict() for smp in students_modules_progresses]
...
def save_result(student_progress_report: StudentProgressReport,
                task_result: str,
                task_message: str) -> StudentProgressReport:
